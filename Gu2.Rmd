...
### Collapse all three datasets to gene level for cross-platform comparison
...
```{r cross-reference2, echo=FALSE, results='asis'}

# Load DE genes
gu.reanlaysis <- read.delim(file.path(dataDir, 'gu_reanalysis.txt'), header=T, sep="\t", row.names=1, as.is=T)

# Pull out top genes and find the overlap
g.geo <- rep(0, nrow(gene_list))
g.geo[which(rownames(gene_list) %in% rownames(top.genes))] <- 1
g.gu <- rep(0, nrow(gene_list))
g.gu[which(rownames(gene_list) %in% rownames(gu.reanlaysis))] <- 1

overlap <- (cbind(as.logical(g.geo), as.logical(g.gu)))
rownames(overlap) <- rownames(gene_list)
colnames(overlap) <- c("GSE9202", "Gu Dataset")

vennDiagram(overlap)

# Use ROC to see where the Gu genes lie within the ranking
ordered <- gene_list[order(gene_list$adj.P.Val), ]
roc <- rocRanked(rownames(gu.reanlaysis), ordered)
...
roc <- rocRanked(gu.reanlaysis$Gene.Symbol, ordered)
